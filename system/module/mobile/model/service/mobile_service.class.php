 ...
 *      tel:400-600-2042
 */
class mobile_service extends service {
    protected $result;


    public function _initialize() {
	$this->vcode_table = $this->load->table('vcode');
        $this->model = $this->load->table('mobile/member');
    ...
    public function init() {      
        ...
        $sqlmap = array();
        $sqlmap['token'] = $key;                
        $result = $this->mb_token_table->where($sqlmap)->find();
     ...
     * 退出登陆
     */
    public function logout($memberinfo) {

        $model_mb_user_token = Model('mb_user_token');

        if($memberinfo['username'] == $_POST['username']) {
            $condition = array();
            $condition['member_id'] = $memberinfo['id'];
            $this->mb_token_table->where($condition)->delete();
     ...
     */
    public function ajax_member_info($param)
    {
        $member =  $this->model->where($param)->find();        
        return $member;
    }
