...
class order_service extends service {
	...
	public function create($buyer_id = 0, $skuids = 0, $district_id, $pay_type = 1,$deliverys = array(), $order_prom = array(), $sku_prom = array(), $remarks = array(), $invoices = array(), $submit = false) {
		...
		$deliverys_default = $this->table_delivery_template->where(array('isdefault' => 1,'enabled' => 1))->find();
		...
		foreach ($carts['skus'] as $seller_id => $value) {
			...
			foreach ($value['sku_list'] as $sku_id => $sku) {
				$_deliverys = $this->_create_sku_delivery($sku,$deliverys_default['id']);
				if(isset($_deliverys['template'])){
					$deliverys['template'][$_deliverys['delivery_template_id']][] = $_deliverys['template'];
				}else{
					$deliverys['price'][] = $_deliverys['price'];
				}
				/* 获取商品促销信息 */
				$sku['_promos'] = array();
				$sku['_give'] = array();
			...
			/* 运费计算 */
			$delivery_price = $this->_delivery_fee($deliverys,$district_id,$setting['delivery_fee_type']);
	...
	/**
	 * 组装sku物流数据
	 * @param  array $sku  商品信息
	 * @param  int $default_delivery_id  默认运费模板id
	 * @return [decimal]
	 */
	private function _create_sku_delivery($sku = array(),$default_delivery_id = 0){
		$delivery = array();
		if($sku['_sku_']['delivery_type'] == 1){
			$delivery = $sku['_sku_']['delivery_price'];
			return array('price' => $delivery);
		}else{
			if($sku['_sku_']['delivery_template_id'] > 0 && $this->table_delivery_template->find($sku['_sku_']['delivery_template_id'])){
				$delivery_template_id = $sku['_sku_']['delivery_template_id'];
			}else{
				$delivery_template_id = $default_delivery_id;
			}
			$delivery = array(
				'number' => $sku['number'],
			...
			return array('template' => $delivery,'delivery_template_id' => $delivery_template_id);
		}
	}
	/**
	 * 计算运费
	 ...
	 * @param  int $type  运费计算模式，0为累计，1为组合，默认累计
	 * @return [decimal]
	 */
	private function _delivery_fee($deliverys = array(),$district_id = 0,$type = 0){
		if((empty($deliverys)) || (int) $district_id < 1){
			$this->error = lang('_param_error_');
			return FALSE;
		}
		$total_fee = 0; //配送费
		$total_first_fee = array(); //首费数组
		foreach ($deliverys['template'] as $delivery_id => $delivery) {
			...
			if($value > $template['first_value']){
				$total_fee += ceil(($value - $template['first_value'])/$template['follow_value']) * $template['follow_fee'];
			}
			$total_first_fee[] = $template['first_fee'];
		}
		//首费替换与计算
		$total_fee += $type == 0 ? array_sum($total_first_fee) : max($total_first_fee);
		//统一运费计算
		$delivery_price = max($deliverys['price']);
		//最终运费
		$total_fee = $type == 0 ? ($total_fee + $delivery_price) : max($total_fee,$delivery_price);
		return $total_fee;
	}
	/**
	 * 创建子订单
